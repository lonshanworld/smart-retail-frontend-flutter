...
class PrinterSettingsController extends GetxController {
  ...
  void onInit() {
    ...
    print('⚙️ [PRINTER SETTINGS CONTROLLER] Initialized - Mode: ${_isDev ? "DEV (Mock)" : "PROD (Real)"}');
  ...
  void scan() {
    print('🔍 [PRINTER SETTINGS CONTROLLER] Starting device scan (${_isDev ? "Mock" : "Real"})');
    _printerService.scanForDevices();
    if (_isDev) {
      print('📱 [PRINTER SETTINGS CONTROLLER] Mock devices will be returned for testing');
    }
  }
  ...
  void connect(device) {
    print('🔗 [PRINTER SETTINGS CONTROLLER] Connecting to device: ${device.name} (${_isDev ? "Mock" : "Real"})');
    _printerService.connectToDevice(device);
    if (_isDev) {
      print('✅ [PRINTER SETTINGS CONTROLLER] Mock connection simulated for: ${device.name}');
    }
  }
