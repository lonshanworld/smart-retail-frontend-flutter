...
class PromotionAddEditView extends GetView<PromotionAddEditController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Obx(() => Text(controller.isEditing.value ? 'Edit Promotion' : 'Add Promotion')),
      ),
            ...
            children: [
              ...
              Obx(() => TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: controller.selectedPromotionType.value == 'percentage' 
                    ? 'Discount Percentage *' 
                    : 'Discount Amount *',
                  hintText: controller.selectedPromotionType.value == 'percentage'
                    ? 'e.g., 10 (for 10% off)'
                    : 'e.g., 5 (for \$5 off)',
                  border: const OutlineInputBorder(),
                  prefixText: controller.selectedPromotionType.value == 'percentage' ? '' : '\$',
                  suffixText: controller.selectedPromotionType.value == 'percentage' ? '%' : '',
                ),
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  ...
                  hintText: 'e.g., 50 (promotion applies only if cart total >= this)',
                  border: OutlineInputBorder(),
                  prefixText: '\$',
              ...
              Obx(() => ElevatedButton(onPressed: controller.isSaving.value ? null : controller.savePromotion, child: Text(controller.isSaving.value ? 'Saving...':'Save Promotion')))
            ],
          ),
        ),
      ),
  ...
  Widget _buildShopSelector() {
    return Obx(() {
      ...
      return DropdownButtonFormField<Shop>(
        ...
        items: controller.shopList.map((s) => DropdownMenuItem(value: s, child: Text(s.name))).toList(),
        onChanged: controller.onShopSelected,
        decoration: const InputDecoration(border: OutlineInputBorder(), prefixIcon: Icon(Icons.store_outlined)),
  ...
  Widget _buildPromotionTypeSelector() {
    return Obx(() => DropdownButtonFormField<String>(
      ...
      items: const [
        ...
        DropdownMenuItem(value: 'bogo', child: Text('Buy One Get One (BOGO)')),
      ],
  ...
  Widget _buildApplicabilitySelector() {
    return Obx(() {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const Text('Promotion Applies To:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          RadioListTile<String>(
            title: const Text('All Products (Cart-wide)'),
            subtitle: const Text('Discount applies to entire cart when minimum spend is met'),
            value: 'all',
          ...
          RadioListTile<String>(
            ...
            subtitle: const Text('Discount applies only when selected products are in cart'),
            value: 'specific',
            groupValue: controller.promotionAppliesTo.value,
            onChanged: (value) {
              controller.promotionAppliesTo.value = value!;
              if (value == 'specific' && controller.selectedShop.value != null) {
                controller.fetchProductsForShop(controller.selectedShop.value!.id!);
              }
            },
          ),
          if (controller.promotionAppliesTo.value == 'specific') ...[
            ...
            if (controller.selectedShop.value == null)
              Container(
                ...
                child: const Text('Please select a shop first to choose products'),
              )
            else
  ...
  Widget _buildProductSelector() {
    return Obx(() {
      ...
      if (controller.productList.isEmpty && !controller.isLoadingProducts.value) {
        return Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
      ...
      return DropdownButtonFormField<InventoryItem>(
        ...
        items: controller.productList.map((p) => DropdownMenuItem(value: p, child: Text(p.name))).toList(),
        ...
        Expanded(
          child: InkWell(
            ...
            child: InputDecorator(
              ...
              child: Obx(() => Text(controller.endDate.value == null
                  ? 'Select...'
