...
class InventoryView extends GetView<InventoryController> {
    ...
    return MerchantMainScaffold(
      ...
      body: Container(
        ...
        child: Obx(() {
          ...
          if (controller.errorMessage.value != null && controller.inventoryItems.isEmpty) {
            return Center(
              child: Padding(
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const Text('No inventory items found.', style: TextStyle(fontSize: 18)),
                const SizedBox(height: 10),
                ElevatedButton.icon(
                  icon: const Icon(Icons.add_shopping_cart_outlined),
                ...
                child: ResponsiveDataTable<InventoryItem>(
                  ...
                  buildCells: (item) => [
                    DataCell(
                      ...
                      onTap: () => Get.toNamed(Routes.MERCHANT_INVENTORY_EDIT, arguments: item),
                    ),
                    DataCell(
                      Text(item.sku ?? 'N/A'),
                      onTap: () => Get.toNamed(Routes.MERCHANT_INVENTORY_EDIT, arguments: item),
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: !item.isSynced 
                              ...
                              : (item.isArchived ? Colors.grey.shade200 : Colors.green.shade50),
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            color: !item.isSynced 
                                ...
                                : (item.isArchived ? Colors.grey.shade700 : Colors.green.shade700),
                        ...
                        children: [
                          if (item.isArchived)
                            IconButton(
                              ...
                              tooltip: 'Unarchive',
                              onPressed: () => controller.unarchiveInventoryItem(item.id!),
                            )
                          else
                            IconButton(
                              icon: const Icon(Icons.archive_outlined, color: Colors.orange, size: 20),
                              tooltip: 'Archive',
                              onPressed: () => controller.archiveInventoryItem(item.id!),
                            ),
                          IconButton(
                            icon: Icon(Icons.edit_outlined, color: AppColors.merchant, size: 20),
                            tooltip: 'Edit',
                            onPressed: () => Get.toNamed(Routes.MERCHANT_INVENTORY_EDIT, arguments: item),
                          ),
                        ],
                      ),
                    ),
                  ...
                  buildMobileCard: (item) => DataRowCard(
                    leading: CircleAvatar(
                      ...
                      child: Text(
                        item.name.isNotEmpty ? item.name[0].toUpperCase() : '?',
                        style: TextStyle(
                          color: item.isArchived ? Colors.white : AppColors.merchant,
                        ),
                      ),
                    ),
                    title: item.name,
                    ...
                    details: [
                      ...
                      DetailRow(
                        ...
                        label: 'Status',
                        value: !item.isSynced 
                            ? (item.needsCreate ? 'New (Local)' : 'Modified (Local)') 
                            : (item.isArchived ? 'Archived' : 'Synced'),
                      ),
                    ],
                    trailing: Row(
                      ...
                      children: [
                        if (item.isArchived)
                          IconButton(
                            icon: const Icon(Icons.unarchive_outlined, color: Colors.green, size: 24),
                            onPressed: () => controller.unarchiveInventoryItem(item.id!),
                          )
                        else
                          IconButton(
                            icon: const Icon(Icons.archive_outlined, color: Colors.orange, size: 24),
                            onPressed: () => controller.archiveInventoryItem(item.id!),
                          ),
                        IconButton(
                          icon: Icon(Icons.edit_outlined, color: AppColors.merchant, size: 24),
                          onPressed: () => Get.toNamed(Routes.MERCHANT_INVENTORY_EDIT, arguments: item),
                        ),
                      ],
                    ),
  ...
  Widget _buildPaginationControls() {
    return Obx(() {
      if (controller.totalPagesFromApi.value <= 1 && !controller.isFetchingPage.value) {
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), spreadRadius: 0, blurRadius: 5)],
        ),
        child: Row(
          ...
          children: [
            IconButton(
              ...
              onPressed: controller.currentPage.value > 1 ? () => controller.jumpToPage(1) : null,
              tooltip: 'First Page',
            ),
            IconButton(
            ...
            IconButton(
              ...
              onPressed: controller.currentPage.value < controller.totalPagesFromApi.value ? controller.goToNextPage : null,
              tooltip: 'Next Page',
            ),
            IconButton(
              icon: const Icon(Icons.last_page_outlined),
              onPressed: controller.currentPage.value < controller.totalPagesFromApi.value 
                           ? () => controller.jumpToPage(controller.totalPagesFromApi.value) 
                           : null,
              tooltip: 'Last Page',
            ),
          ],
