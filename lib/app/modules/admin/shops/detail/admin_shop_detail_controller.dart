...
class AdminShopDetailController extends GetxController {
  final ShopApiService _shopApiService =
      Get.find<ShopApiService>(); // <-- SERVICE INSTANCE ADDED
  ...
  void _loadShopDetailsFromArgs() {
    ...
    if (argument is Shop) {
      ...
      errorMessage.value =
          "Could not load shop details. Invalid data received.";
  ...
  void navigateToEditShop() {
    if (shop.value != null) {
      Get.toNamed(Routes.ADMIN_ADD_EDIT_SHOP, arguments: shop.value)?.then((
        result,
      ) {
        if (result == true) {
      ...
      DialogUtils.showError("Shop data not available for editing.");
    }
  }
  ...
  Future<void> refreshShopDetails() async {
    ...
    if (shop.value != null && shop.value!.id != null) {
      ...
      _loadShopDetailsFromArgs();
      if (shop.value == null) {
        // If still null after trying to reload from args
        DialogUtils.showError("Cannot refresh. Shop data is unavailable.");
      }
    }
  }
}
